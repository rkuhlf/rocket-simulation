...
def load_sources_with_converted_units():
    ...
    data.stratologger["altitude"] /= 3.28
    data.stratologger["speed"] /= 3.28
    ...
    data.stratologger["altitude"] += 1192

    # I like F more than C
    data.telemetrum["temperature"] = data.telemetrum["temperature"] * 9/5 + 32

    time_offset = 8056.71
    data.teleGPS = data.teleGPS[data.teleGPS["time"] > time_offset]
    data.teleGPS["time"] -= time_offset
...
def load_sources_with_calculations():
    ...
    speeds = []
    p_row = data.teleGPS.iloc[0]
    for index, row in data.teleGPS.iterrows():
        if index == 0:
            continue
...
def compare_altitude(data):
    plt.plot(data.telemetrum["time"], data.telemetrum["altitude"], label="Telemetrum")
    plt.plot(data.stratologger["time"], data.stratologger["altitude"], label="Stratologger")
    plt.plot(data.teleGPS["time"], data.teleGPS["altitude"], label="TeleGPS")
...
def compare_speed(data):
    plt.plot(data.telemetrum["time"], data.telemetrum["speed"], label="Telemetrum")
    plt.plot(data.stratologger["time"], data.stratologger["speed"], label="Stratologger")
    plt.plot(data.teleGPS["time"], data.teleGPS["speed"], label="TeleGPS")
...
def compare_mach(data):
    plt.plot(data.telemetrum["time"], data.telemetrum["mach"], label="Telemetrum")
    plt.plot(data.stratologger["time"], data.stratologger["mach"], label="Stratologger")
    plt.plot(data.teleGPS["time"], data.teleGPS["mach"], label="TeleGPS")
...
def compare_lat_long(data):
    ...
    ax1.plot(data.telemetrum["time"], data.telemetrum["latitude"], label="Telemetrum")
    ax1.plot(data.teleGPS["time"], data.teleGPS["latitude"], label="TeleGPS")
    ax1.legend()
    ax1.set_title("Latitude")

    ax2 = fig.add_subplot(122)
    ax2.plot(data.telemetrum["time"], data.telemetrum["longitude"], label="Telemetrum")
    ax2.plot(data.teleGPS["time"], data.teleGPS["longitude"], label="TeleGPS")
    ax2.legend()
...
def compare_temperature(data):
    plt.plot(data.telemetrum["time"], data.telemetrum["temperature"], label="Telemetrum")
    plt.plot(data.stratologger["time"], data.stratologger["temperature"], label="Stratologger")
...
def compare_flight_angle(data):
    ...
    plt.plot(data.telemetrum["time"], data.telemetrum["angle"], label="Telemetrum")
    plt.plot(data.teleGPS["time"], data.teleGPS["angle"], label="TeleGPS")
