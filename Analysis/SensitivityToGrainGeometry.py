...
from fluids.friction import friction_factor, helical_turbulent_fd_Prasad, Blasius, friction_factor_curved
import numpy as np
...
def display_helical_effects(helix_diameters=np.linspace(0.01, 10, 200), port_diameters=[0.1]):
    ...
    for port_diameter in port_diameters:
        ...
        plt.plot(helix_diameters, friction_coefficients, label=str(port_diameter))

        if len(port_diameters) == 1:
